...
def get_training_op(loss, args):
    ...
    return train_op, image_pretrain_saver,  global_step
...
def train(args):
    ...
    visfeat_vars = [v for v in t_vars if 'image' in v.name] # only visual embedding layers
    sentfeat_vars = [v for v in t_vars if 'text' in v.name] # only sent embedding layers
    
    

    #Get the training op
    # train_op, image_pretrain_saver, global_step = get_training_op(total_loss, args)
    ...
    # while not sess.should_stop():
        # try:
    summary_val,__,total_loss_val, loss_s_val, loss_im_val, global_step_value, ie, te = sess.run([summary_tensor, emb_train_op,total_loss, loss_s, loss_im, global_step,image_embeddings, text_embeddings])
